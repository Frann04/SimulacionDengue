...
personas-own [
  ...
  duracion-inmunidad
  vacunado?            ;; Nueva variable para controlar si está vacunado
]

;; Procedimiento corregido para usar la variable global
to vacunar-poblacion
  ...
  ifelse count personas-no-vacunadas >= cantidad-a-vacunar [
    ask n-of cantidad-a-vacunar personas-no-vacunadas [
      ...
      set duracion-inmunidad (ticks + 21 + random 21)
    ]
  ] [
...
to setup
  ...
  create-mosquitos cant-mosquitos [
    ...
    set shape "bicho"
    set ya-pico? false
    set tick-de-muerte ticks + 30 + random 60
...
to go
    ...
    if count(charcos-here) > 0 [
      ...
      ask charco-mas-cercano [
        if not incubando? [
          set color 75
          set incubando? true
          set tick-de-incubacion ticks + 24 + random 15
  ...
  ask personas with [color = green or color = violet] [
    if (count (mosquitos-here with [color = orange and not ya-pico?]) > 0) [
      ifelse vacunado? [
        ;; Si está vacunado, tiene 80% de probabilidad de NO infectarse
        if random 100 < chance-de-infeccion-vacunado [  ;; Solo 20% del chance normal
          set duracion-de-enfermedad (ticks + 21 + random 21)
          set color red
        ]
      ] [
        ;; Si no está vacunado, probabilidad normal
        if random 100 < chance-de-infeccion-sin-vacunar [
          set duracion-de-enfermedad (ticks + 21 + random 21) ;; entre 7 y 14 dias
          set color red
        ]
      ]
    ]
...
to curar-gente
  ask personas with [color = red] [
    if duracion-de-enfermedad <= ticks [
      ifelse vacunado? [
...
to terminar-inmunidad
  ask personas with [color = violet] [
    if (ticks - duracion-inmunidad) >= 150 [  ; Check if 21 ticks (days) have passed since vaccination
